...
class RainMachineProgramSunOffsetNumber(CoordinatorEntity, NumberEntity):
    ...
    a pending value applied when a sun mode is selected."""
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if prog:
            ...
            if int(stp.get("type", 0)) != 0:
                key = f"{self._entry.entry_id}_prog_{self._pid}_sun_offset_min"
                self.hass.data[DOMAIN][key] = int(stp.get("offsetMinutes", 0))
    ...
    def native_value(self) -> float:
        ...
        if prog:
            stp = prog.get("startTimeParams", {})
            if int(stp.get("type", 0)) != 0:
                return float(stp.get("offsetMinutes", 0))
        return float(self.hass.data[DOMAIN].get(
