...
class RainMachineClient:
    ...
    async def action_set_program_start_time(self, pid: int, start_time: str) -> dict:
        ...
        clearing any sunrise/sunset-relative startTimeParams (same coupling as
        the device web UI radio buttons)."""
        async with aiohttp.ClientSession() as session:
            await self.authenticate(session)
            data = await self._get(session, f"program/{pid}")
    ...
    async def action_set_program_start_time_params(
        ...
        The device recomputes startTime from sunrise/sunset when type != 0."""
        async with aiohttp.ClientSession() as session:
            await self.authenticate(session)
            data = await self._get(session, f"program/{pid}")
    ...
    async def action_adjust_program_durations(
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for wt in watering_times:
                ...
                if fixed_dur > 0:
                    current_pct = fixed_dur / ref_time
                else:
